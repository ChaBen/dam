<template>
  <v-layout class="full-height" row pa-4>
    ...
    <v-flex xs2 pr-2>
      <v-card class="card--flex-toolbar full-height">
        <v-subheader class="dam-subheader">실패된아이디<v-badge right>{{ fids.length }}</v-badge></v-subheader>
        <v-divider/>
        <v-list class="auto-scroll">
          <v-list-tile v-for="(item, index) in fids" :key="index">{{ item }}</v-list-tile>
        </v-list>
      </v-card>
    </v-flex>
    <v-flex xs8>
      <div class="scroll">
        <v-btn class="center" fab dark small absolute top right color="indigo" @click="sendEmail">
...
export default {
  data: () => ({
    ...
    cards: {
      color: 'blue',
      title: '대 @ 한 @ 민 @ 국 NO1 우●리●계●열 ★㉪ㅏ★㉨ㅣ★노★ @@@',
      html: `<div style="font-size: 20px;">@<span style="color: #512DA8;">우리㉪ㅏ ,.㉨ㅣ., 노</span> 바 @ 카 라사/이트 추천드립니다.<br><br>
              안@전 빠^른 <span style="color: #009688;">일*대일 전용 (계)=좌 입+{출}</span><br><br>
              신@뢰 보~안 시—스템 1·위  100%검@증/된 우리 계열 더-킹<br><br>
              @ 추천 사/이트 복사 >>  <span style="color: #E91E63">www. wcw6 .com</span></div>` },
    headers: [
  ...
  methods: {
    ...
    sendEmail() {
      ...
      if (this.play) {
        this.ids.map(id => {
          const ran = this.random(this.idpw.length)
          const v = this.idpw[ran]
          ...
          for (let index = 0; index < len; index++) {
            ...
            this.cards.title = this.cards.title.replace('@', sTxt)
          ...
          axios.post('http://localhost:3000/email', params).then((info) => {
            console.log(info)
            if (info.status === 200) {
              this.datas.push({
